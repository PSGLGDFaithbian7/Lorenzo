...
[25:18]   last_head_count         最后一批 head 数，host 预先算好，1..Hp_parallel，且不超过 num_heads
[17:0]    reserved / implementation-defined extension
```
...
文中 `active_sa_count` 表示当前 head tile 实际启用的 SA 数（1..4）。QK 中它也等于
QP/input_factor 的有效标量数；PV 中 QP/input_factor 的有效标量数是当前 tile 的 head 数，
full tile 为 `Hp_parallel`，最后 partial tile 为 `last_head_count`，而 SA 启用数为
`active_head_count * SA_per_head`。
...
- **QP 共享流**：在 PV mode 下表示 `P`，按 `t = 0 .. context_length-1` 顺序输入，每拍送
  当前 head tile 的 active head 数个 FP8 标量；full tile 为 `Hp_parallel`，最后 partial tile
  为 `last_head_count`
- **input factor 流**：与 QP 共享流同拍输入，每拍送相同数量的 FP8 因子：
  `FP(H+0,t), FP(H+1,t), ...`
