//==============================================================================
// tb_lte_boundary_ctrl.sv - B4 boundary controller directed test
//==============================================================================
`timescale 1ns/1ps
import lte_pkg::*;
...
module tb_lte_boundary_ctrl;
  ...
  task automatic chk_i(input string nm, input int got, exp);
    if (got!==exp) begin
      $display("[FAIL] t=%0t %s got=%0d exp=%0d", $time, nm, got, exp);
      err_cnt++;
    end
  endtask
  ...
  task automatic chk32(input string nm, input logic [31:0] got, exp);
    if (got!==exp) begin
      $display("[FAIL] t=%0t %s got=%08x exp=%08x", $time, nm, got, exp);
      err_cnt++;
    end
  endtask
  ...
  initial begin
    ...
    // TC1: task_start remains combinational for initial clear events.
    task_mode = TASK_MODE_QK;
    task_start=1; #1;
    chk1("tc1_gs_start",  group_start,     1'b1);
    chk1("tc1_hts_start", head_tile_start, 1'b1);
    chk1("tc1_rs_start",  row_start,       1'b1);
    @(posedge clk); #1; task_start=0; #1;
    chk1("tc1_gs_drop", group_start, 1'b0);
    $display("[PASS] TC1: task_start events");

    // TC2: group_done_fire is registered into group_done_pulse/deq token.
    ...
    group_done_fire=1; #1;
    chk1("tc2_no_combo_gdp", group_done_pulse, 1'b0);
    @(posedge clk); #1; group_done_fire=0;
    chk1("tc2_gdp",          group_done_pulse,    1'b1);
    chk1("tc2_deq_valid",    deq_token_valid,     1'b1);
    chk_i("tc2_deq_head",    deq_token_meta.head_tile_id, 2);
    chk_i("tc2_deq_group",   deq_token_meta.group_id, 3);
    ...
    clk_tick(1);
    chk1("tc2_gdp_one_pulse", group_done_pulse, 1'b0);
    $display("[PASS] TC2: registered group_done token");

    // TC3: qk_block_done_fire registers row_done and QK drain token.
    task_mode = TASK_MODE_QK;
    group_done_last_group=1'b1;
    group_done_lane_valid_mask = 32'h0000_000F;
    qk_block_done_fire=1; #1;
    chk1("tc3_no_combo_row", row_done_pulse, 1'b0);
    ...
    // TC4: head_step_fire registers head_tile_done and starts next tile one cycle later.
    task_done_fire=0; head_step_fire=1; #1;
    chk1("tc4_no_combo_hdone", head_tile_done_pulse, 1'b0);
    @(posedge clk); #1; head_step_fire=0;
    chk1("tc4_hdone",       head_tile_done_pulse, 1'b1);
    chk1("tc4_hts_on_step", head_tile_start, 1'b1);
    clk_tick(1);
    $display("[PASS] TC4: head_tile_done");

    // TC5: task_done_fire registers task_done_pulse and suppresses new starts.
    group_done_fire=1; task_done_fire=1; #1;
    chk1("tc5_no_combo_done", task_done_pulse, 1'b0);
    @(posedge clk); #1; group_done_fire=0; task_done_fire=0;
    chk1("tc5_task_done", task_done_pulse, 1'b1);
    chk1("tc5_no_gs",     group_start,     1'b0);
    clk_tick(1);
    $display("[PASS] TC5: task_done, no new group_start");

    // TC6: PV mode uses head_step_fire, not qk_block_done_fire, for drain.
    task_mode = TASK_MODE_PV;
    pv_last_inner_count = 6'd32;
    group_done_lane_valid_mask = 32'hFFFF_FFFF;
    qk_block_done_fire=1; @(posedge clk); #1; qk_block_done_fire=0; #1;
    chk1("tc6_no_row_done",     row_done_pulse,    1'b0);
    chk1("tc6_no_drain_pv_qkb", drain_token_valid, 1'b0);
    head_step_fire=1; @(posedge clk); #1; head_step_fire=0;
    chk1("tc6_pv_drain_valid", drain_token_valid, 1'b1);
    chk1("tc6_pv_drain_mode",  drain_token_meta.mode, 1'b1);
    chk32("tc6_pv_drain_mask", drain_token_meta.lane_valid_mask, 32'hFFFF_FFFF);
    clk_tick(1);
    $display("[PASS] TC6: PV mode events");

    // TC7: dynamic use_input_lorenzo remains combinational.
    task_mode = TASK_MODE_QK;
    ...
    chk1("tc7_tail_lz_off",  use_input_lorenzo, 1'b0);
    flag_in_lz_full_only=0; #1;
    chk1("tc7_always_lz_on", use_input_lorenzo, 1'b1);
    $display("[PASS] TC7: dynamic use_input_lorenzo");

    // TC8: PV full_pe_active depends on last group and pv_last_inner_count.
    task_mode = TASK_MODE_PV;
    flag_in_lz=1; flag_in_lz_full_only=1;
    group_done_last_group = 1'b1;
